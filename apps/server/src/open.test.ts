...
it.layer(NodeServices.layer)("launchDetached", (it) => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `matcha-no-such-command-${Date.now()}`,
        args: [],
      }).pipe(Effect.result);
...
it.layer(NodeServices.layer)("isCommandAvailable", (it) => {
  it.effect("resolves win32 commands with PATHEXT", () =>
    Effect.gen(function* () {
      ...
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "matcha-open-test-" });
  ...
  it.effect("uses platform-specific PATH delimiter for platform overrides", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const firstDir = yield* fs.makeTempDirectoryScoped({ prefix: "matcha-open-test-" });
      const secondDir = yield* fs.makeTempDirectoryScoped({ prefix: "matcha-open-test-" });
...
it.layer(NodeServices.layer)("resolveAvailableEditors", (it) => {
  it.effect("returns installed editors for command launches", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "matcha-editors-" });
