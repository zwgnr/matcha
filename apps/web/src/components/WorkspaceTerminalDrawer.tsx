...
function maxDrawerHeight(): number {
  if (typeof window === "undefined") return DEFAULT_WORKSPACE_TERMINAL_HEIGHT;
  return Math.max(MIN_DRAWER_HEIGHT, Math.floor(window.innerHeight * MAX_DRAWER_HEIGHT_RATIO));
}

function clampDrawerHeight(height: number): number {
  const safeHeight = Number.isFinite(height) ? height : DEFAULT_WORKSPACE_TERMINAL_HEIGHT;
...
function TerminalViewport({
  ...
  useEffect(() => {
    ...
    const sendTerminalInput = async (data: string, fallbackError: string) => {
      ...
      try {
        await api.terminal.write({ workspaceId, terminalId, data });
      } catch (error) {
  ...
  }, [drawerHeight, resizeEpoch, terminalId, workspaceId]);
  return (
    <div ref={containerRef} className="relative h-full w-full overflow-hidden rounded-[4px]" />
  );
}

interface WorkspaceTerminalDrawerProps {
  workspaceId: WorkspaceId;
  cwd: string;
  worktreePath?: string | null;
  runtimeEnv?: Record<string, string>;
...
export default function WorkspaceTerminalDrawer({
  workspaceId,
  cwd,
  worktreePath,
  runtimeEnv,
  ...
  const normalizedTerminalIds = useMemo(() => {
    ...
    return cleaned.length > 0 ? cleaned : [DEFAULT_WORKSPACE_TERMINAL_ID];
  }, [terminalIds]);

  const resolvedActiveTerminalId = normalizedTerminalIds.includes(activeTerminalId)
    ? activeTerminalId
    : (normalizedTerminalIds[0] ?? DEFAULT_WORKSPACE_TERMINAL_ID);
    ...
    for (const terminalGroup of terminalGroups) {
      ...
      const baseGroupId =
        terminalGroup.id.trim().length > 0
          ...
          : `group-${nextTerminalIds[0] ?? DEFAULT_WORKSPACE_TERMINAL_ID}`;
      nextGroups.push({
  ...
  return (
    <aside
      className={
        isInline
          ? "workspace-terminal-drawer relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background"
          : "workspace-terminal-drawer relative flex min-w-0 shrink-0 flex-col overflow-hidden border-t border-border/80 bg-background"
      }
