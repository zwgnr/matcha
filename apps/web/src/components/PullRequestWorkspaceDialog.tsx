import type { GitResolvePullRequestResult, WorkspaceId } from "@matcha/contracts";
...
interface PullRequestWorkspaceDialogProps {
  open: boolean;
...
export function PullRequestWorkspaceDialog({
  open,
  workspaceId,
  cwd,
  initialReference,
  onOpenChange,
  onPrepared,
}: PullRequestWorkspaceDialogProps) {
  ...
  const preparePullRequestWorkspaceMutation = useMutation(
    gitPreparePullRequestWorkspaceMutationOptions({ cwd, queryClient }),
  );
  ...
  const handleConfirm = useCallback(
    async (mode: "local" | "worktree") => {
      ...
      try {
        const result = await preparePullRequestWorkspaceMutation.mutateAsync({
          reference: parsedReference,
          mode,
          ...(mode === "worktree" ? { workspaceId } : {}),
        });
        await onPrepared({
    ...
    [
      ...
      onPrepared,
      parsedReference,
      preparePullRequestWorkspaceMutation,
      resolvedPullRequest,
      workspaceId,
    ],
  );
  ...
  const errorMessage =
    ...
    (resolvedPullRequest === null && resolvePullRequestQuery.isError
      ...
      : preparePullRequestWorkspaceMutation.error instanceof Error
        ? preparePullRequestWorkspaceMutation.error.message
        : preparePullRequestWorkspaceMutation.error
          ? "Failed to prepare pull request workspace."
          : null);
  ...
  return (
    <Dialog
      ...
      onOpenChange={(nextOpen) => {
        if (!preparePullRequestWorkspaceMutation.isPending) {
          onOpenChange(nextOpen);
        }
      }}
    >
      <DialogPopup className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Resolve a GitHub pull request, then create the draft workspace in the main repo or in a
            dedicated worktree.
          </DialogDescription>
        </DialogHeader>
          ...
          <label className="grid gap-1.5">
            ...
            <Input
              ...
              onKeyDown={(event) => {
                ...
                if (!isResolving && !preparePullRequestWorkspaceMutation.isPending) {
        ...
        <DialogFooter>
          <Button
            ...
            disabled={preparePullRequestWorkspaceMutation.isPending}
          >
            Cancel
          </Button>
          <Button
            ...
            disabled={
              ...
              !resolvedPullRequest ||
              isResolving ||
              preparePullRequestWorkspaceMutation.isPending
            }
          >
