...
const BASE_DIR = process.env.MATCHA_HOME?.trim() || Path.join(OS.homedir(), ".matcha");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "matcha";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "Matcha (Dev)" : `Matcha v${app.getVersion()}`;
...
const USER_DATA_DIR_NAME = isDevelopment ? "matcha-dev" : "matcha";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Matcha (Dev)" : "Matcha (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.MATCHA_PORT;
  delete env.MATCHA_AUTH_TOKEN;
  delete env.MATCHA_MODE;
  delete env.MATCHA_NO_BROWSER;
  delete env.MATCHA_HOST;
  delete env.MATCHA_DESKTOP_WS_URL;
  return env;
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { matchaCommitHash?: unknown };
    return normalizeCommitHash(parsed.matchaCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.MATCHA_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Matcha failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.MATCHA_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Matcha ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Matcha (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`matcha`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.MATCHA_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.MATCHA_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.MATCHA_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
